...
def convert_examples_to_features(examples, max_seq_len, tokenizer,
    ...
    for ex_index, example in enumerate(examples):
        ...
        assert len(input_ids) == max_seq_len, 'Error with input length {} vs {}'\
            .format(len(input_ids), max_seq_len)
        assert len(attention_mask) == max_seq_len, 'Error with attention mask length {} vs {}'\
            .format(len(attention_mask), max_seq_len)
        assert len(token_type_ids) == max_seq_len, 'Error with token type length {} vs {}'\
            .format(len(token_type_ids), max_seq_len)
        assert len(slot_labels_ids) == max_seq_len, 'Error with slot labels length {} vs {}'\
            .format(len(slot_labels_ids), max_seq_len)
        ...
        features.append(InputFeatures(input_ids=input_ids, attention_mask=attention_mask,
                                      token_type_ids=token_type_ids, intent_label_id=intent_label_id,
